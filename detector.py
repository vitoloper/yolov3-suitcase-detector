...
read_dir = time.time()  # read_dir is a checkpoint used to measure time
#Detection phase
try:
...
load_batch = time.time()    # time checkpoint
...
#load the image 
    ...
    # NOTE prediction[:, 5] is objectness score (box confidence score)
...
# Clip any bboxes that may have boundaries outside the imageto the edges of our image.
for i in range(output.shape[0]):
    output[i, [1,3]] = torch.clamp(output[i, [1,3]], 0.0, im_dim_list[i,0])
    output[i, [2,4]] = torch.clamp(output[i, [2,4]], 0.0, im_dim_list[i,1])
    
# palette of different colors (bounding boxes colors)
...
# Prefix det_ in front of the saved image name
